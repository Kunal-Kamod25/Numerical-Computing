...
vector<long double> SLE::solve(SolverType method, LUMethod lu_m, int max_iterations, long double tolerance) {
    
    // ==================== ORIGINAL METHODS (UNCHANGED) ====================
    
    if (method == GAUSSIAN) {
        // Original Gaussian Elimination code (unchanged)
        GaussianElimination ge(*this);
        ge.upperTriangularWithPivot();
        solution_vec = ge.backSubstitutionToVec();
    } 
    else if (method == LU_METHOD) {
        // Original LU Decomposition code (unchanged)
        int n = rows;
        Matrix A(n, n);
        Matrix B(n, 1);
        for(int i=0; i<n; i++) {
    ...
    // ==================== NEW ITERATIVE METHODS ====================
        ...
        // NEW: Jacobi iterative method
        JacobiSolver solver(*this);
        solver.setMaxIterations(max_iterations);
        solver.setTolerance(tolerance);
        solution_vec = solver.solve();
        cout << "Jacobi converged in " << solver.getIterationCount() << " iterations.\n";
    }
    else if (method == GAUSS_SEIDEL) {
        // NEW: Gauss-Seidel iterative method
        GaussSeidelSolver solver(*this);
        solver.setMaxIterations(max_iterations);
        solver.setTolerance(tolerance);
        solution_vec = solver.solve();
        cout << "Gauss-Seidel converged in " << solver.getIterationCount() << " iterations.\n";
    }
...
Matrix SLE::solveWithoutPivot() {
    GaussianElimination ge(*this);
    ge.upperTriangularWithoutPivot();
    solution_vec = ge.backSubstitutionToVec();
    Matrix res(rows, 1);
    for(int i=0; i<rows; i++) res(i, 0) = solution_vec[i];
    return res;
